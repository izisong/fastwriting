...
var DEFAULT_TEXT = "write your article";
...
function search_changed(e, kd, toroot)
{
	 ...
	 *我要在这里加入ajax技术，对输入分词
	 */
	// 32 -- space     8 -- BackSpace BackSpace
		...
		xmlhttp.onreadystatechange=function()
		{
		  if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				var sentences = xmlhttp.responseText.split("|");
				//document.getElementById("myDiv").innerHTML=xmlhttp.responseText;
				var txt = sentences.length+" matchers"+"<br>"
				for (i=0;i<sentences.length;i++)
				  {
					txt=txt + sentences[i] + "<br>";
				  }
				document.getElementById("myDiv").innerHTML=txt;
			}
		}
		xmlhttp.open("GET","/match/柴静/"+text, true);
		//xmlhttp.open("GET","/test/柴静/"+text, true);
		xmlhttp.send();
