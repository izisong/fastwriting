...
function search_changed(e, kd, toroot)
{
	...
	else if (!kd && ((e.keyCode == 32) || (e.keyCode == 8))) {
		var dataStr = "";
		var cutWords = new Array();
		var matchedSens = new Array();
		var xmlhttp=new XMLHttpRequest();
		xmlhttp.onreadystatechange=function()
		{
		  if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				dataStr=xmlhttp.responseText;
				var dataJSON = eval("(" + dataStr + ")");
				cutWords = dataJSON.cutWords;
				matchedSens = dataJSON.matchedSens;
				
				gMatches = matchedSens;
				matchedCount = matchedSens.length;
				gSelectedIndex = -1;
				...
				highlight_autocomplete_result_labels(cutWords);
				sync_selection_table(toroot);
			}
		}
        xmlhttp.open("GET","/match/Memo/"+text, true);
		xmlhttp.send();
...
function highlight_autocomplete_result_labels(query) {   
	...
	//console.log(queryAlnumDot);
    //var queryRE = new RegExp(
    //    '(' + queryAlnumDot.replace(/\./g, '\\.') + ')', 'ig');
	var queryRE = new RegExp(
        '(' + query.toString().replace(/,/g, '|') + ')', 'ig');
	console.log(queryRE);
    for (var i=0; i<gMatches.length; i++) {
